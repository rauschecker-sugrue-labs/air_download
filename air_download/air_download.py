...
def parse_args():
    ...
    parser.add_argument(
        'url',
        help='URL for AIR API, e.g. https://air.<domain>.edu/api/'
        )
    parser.add_argument(
        'acc',
        metavar='ACCESSION',
        help='Accession # to download'
        )
    parser.add_argument(
        '-c',
        '--cred-path',
...
def authenticate(url, cred_path):
    if cred_path:
        assert os.path.exists(cred_path), f'AIR credential file ({cred_path}) does not exist.'
...
def download(url, cred_path, accession, output, project, profile, series_inclusion=None):
    jwt, _ = authenticate(url, cred_path)
    header = {'Authorization': 'Bearer ' + jwt}

    # Search for study by accession number 
    study = requests.post(urljoin(url, 'secure/search/query-data-source'), 
        headers = header,
        json = {'name': '',
                'mrn': '',
                'accNum': accession,
    ...
    series = requests.post(urljoin(url, 'secure/search/series'),
        headers = header,
        json = study).json()

    # Filter series based on inclusion patterns, if provided
    if series_inclusion:
        inclusion_list = [pattern.strip().lower()
                          for pattern in series_inclusion.split(',')]
        original_series_description = [serie["description"] for serie in series]
        series = [serie for serie in series 
                if (serie["description"] and any(pattern in serie["description"].lower()
...
def main(args):
    download(
        args.url, args.cred_path,
        args.acc, args.output,
